...
def stress_bar_adjustment(preprocessor, file: str, group_files: str, flags: dict):
    ...
    cs = sigma_stretch_cs(preprocessor.vertical_options["theta_s"],
                          preprocessor.vertical_options["theta_b"], sc, preprocessor.vertical_options["sigma_type"])
    hc = preprocessor.hc
    ...
    with netCDF4.Dataset(file, mode='r+') as my_file:
        ...
        ubar: netCDF4.Variable = my_file.createVariable('ubar', 'f', ('time', 'eta_rho', 'xi_u'))
        vbar: netCDF4.Variable = my_file.createVariable('vbar', 'f', ('time', 'eta_v', 'xi_rho'))
        ...
        for t in range(time_len):
            ...
            ubar_vals, vbar_vals = get_obcvolcons(ubar_vals, vbar_vals, pm, pn, rmask, flags['obc'],
                                                  preprocessor.verbose)
...
clim_adjustments = [{'out_var_names': {'ubar', 'vbar'}, 'in_var_names': {'u', 'v'}, 'flags': 'obc',
                     'func': stress_bar_adjustment}]
